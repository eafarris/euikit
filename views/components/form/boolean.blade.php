    ...
    @if ($type == 'radio')
        ...
        <div class="flex mt-1">
            <div class="flex items-center mb-4 mr-3">
                <input class="h-4 w-4 border-slate-300 focus:ring-2 focus:ring-indigo-500" type="radio" name="{{ $field }}" id="{{ $field }}_yes" value="1" {{ $value ? 'checked' : '' }} {{ $attributes->whereStartsWith('wire') }}>
                <label for="{{ $field}}_yes" class="text-sm font-medium text-slate-900 ml-2 block">Yes</label>
            </div>
            <div class="flex items-center mb-4">
                <input class="h-4 w-4 border-slate-300 focus:ring-2 focus:ring-indigo-500" type="radio" name="{{ $field }}" id="{{ $field }}_no" value="0" {{ $value ? '' : 'checked' }} {{ $attributes->whereStartsWith('wire')}}>
